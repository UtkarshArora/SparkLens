...
def compute_null_counts(df: DataFrame) -> Dict[str, int]:
    ...
    if not df.columns:
        return {}
    agg_exprs = [
        F.sum(F.when(is_blank_or_null(c), 1).otherwise(0)).alias(c) for c in df.columns
    ]
...
# ----------------------------------------------------------------------
# Schema-profiling helpers (Utkarsh)
# ----------------------------------------------------------------------
...
def resolve_type(type_str: str):
    """Map a schema-config type string to its Spark type class."""
